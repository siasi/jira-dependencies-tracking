...
2. The idea is to follow a simple process using this toolking:
    1. extract data and store locally (the jira_scan.py script)
...
3. Currently the jira_scan.py has flexibility to filter data through options (quarter, status, jql). However there are 2 real usage (see the list). 
